...
def preprocess_all_raw_data(raw_dir="data/raw", output_csv="data/processed/dataset.csv"):
    """
    ...
    if not csv_files:
        print(f"お知らせ: {raw_dir} 内に sign_language_*.csv ファイルが見当たらなかったため、処理をスキップしました。データを data/raw に配置してから実行してください。")
        return
    ...
    if all_data:
        ...
        print(f"データセット作成完了: 全 {len(dataset_df)} フレームの正規化データを {output_csv} に統合しました。")
    else:
